	...
	<?php if (have_posts()) : ?>
		...
		<?php while (have_posts()) : the_post(); ?>
			...
			<div <?php post_class(depo_post_category()) ?> id="post-<?php the_ID(); ?>">
				...
				<p class="meta">In <?php the_category(','); ?> on <?php the_time(get_option('date_format')) ?> <?php _e('by'); ?> <?php the_author() ?> <?php the_tags(__('Tagged:', 'depo-square') . ' ', ', ', ''); ?> <?php edit_post_link(__('Edit', 'depo-square'), '', ''); ?></p>
		...
		<div class="navigation_to_home"><a href="<?php bloginfo('wpurl'); ?>"><?php _e('&laquo Back to Home'); ?></div>
