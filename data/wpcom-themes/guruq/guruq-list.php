...
$edit_link = add_query_arg( array( 'page' => GURUQ_SLUG ), 'edit.php' );
...
if ( isset($_GET['doaction']) || isset($_GET['doaction2']) || isset($_GET['delete_all']) || isset($_GET['delete_all2']) || isset($_GET['bulk_edit']) ) {
	...
	if ( isset($_GET['delete_all']) || isset($_GET['delete_all2']) ) {
		...
		wp_redirect( admin_url( $edit_link ) );
	}
...
$cat = get_category_by_slug( GURUQ_CAT );
$total = (int) $cat->category_count;
$pagenum = 1;
$per_page = 20;
$num_pages = (int) ( $total / $per_page );

// set array of args for get_posts
$post_args = array();
if ( isset( $_GET['pagenum'] ) )
	$post_args['pagenum'] = intval( $_GET['pagenum'] ) > 0 ? intval( $_GET['pagenum'] ) : 1;
if ( isset( $_GET['s'] ) )
	$post_args['s'] = $_GET['s'];
if ( isset( $_GET['post_status'] ) ) {
	$post_args['post_status'] = $_GET['post_status'];
} else {
	// Filter out post_status
	add_filter( 'pre_get_posts', 'guruq_remove_post_status' );
}

$post_args['numberposts'] = $per_page;
$post_args['category'] = $guruq_cat_id;
$post_args['post_type'] = 'post';

$posts = get_posts( $post_args );
?>

<form id="posts-filter" action="<?php echo $edit_link; ?>" method="get">

<ul class="subsubsub">
<?php
if ( empty($locked_post_status) ) :
$status_links = array();
$num_posts = guruq_count_posts( 'post', 'readable', GURUQ_CAT );
$class = '';
$allposts = '';
...
if ( $user_posts ) {
	if ( $_GET['author'] == $current_user->ID )
		$class = ' class="current"';
	$status_links[] = "<li><a href='" . add_query_arg( array( 'author' => $current_user->ID ), $edit_link ) . "'$class>" . sprintf( _nx( 'My Posts <span class="count">(%s)</span>', 'My Posts <span class="count">(%s)</span>', $user_posts_count, 'posts' ), number_format_i18n( $user_posts_count ) ) . '</a>';
...
$status_links[] = "<li><a href='$edit_link{$allposts}'$class>" . sprintf( _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $total_posts, 'posts' ), number_format_i18n( $total_posts ) ) . '</a>';
...
foreach ( $post_stati as $status => $label ) {
	...
	$edit_link = add_query_arg( array( 'post_status' => $status ), $edit_link );
	$status_links[] = "<li><a href='$edit_link'$class>" . sprintf( _n( $label[2][0], $label[2][1], $num_posts->$status ), number_format_i18n( $num_posts->$status ) ) . '</a>';
}
echo implode( " |</li>\n", $status_links ) . '</li>';
unset( $status_links );
endif;
...
<p class="search-box">
	<input type="hidden" name="page" value="<?php echo GURUQ_SLUG; ?>" />
