...
class Tests_Post_Revisions extends WP_UnitTestCase {
	...
	*/
	function test_revision_dont_save_revision_if_unchanged() {
		...
		$this->assertCount( 1, wp_get_post_revisions( $post_id ) ); //should be 1 revision so far

		//update the post
		wp_update_post( array( 'post_content'	=> 'new update for some updated content', 'ID' => $post_id ) );	//2nd revision
		$this->assertCount( 2, wp_get_post_revisions( $post_id ) ); //should be 2 revision so far
		...
		$this->assertCount( 2, wp_get_post_revisions( $post_id ) ); //should still be 2 revision
		...
		$this->assertCount( 3, wp_get_post_revisions( $post_id ) ); //should  be 3 revision
